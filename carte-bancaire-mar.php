...
if(isset($_SESSION['marque']) && !isset($_SESSION['pseudo'])){
    ...
    $req=$db->query("SELECT * FROM cartes WHERE pseudo='$marque' LIMIT 1");
    $res=$req->fetch();
    if ($res) {
        $oldname=$res['nom'];
    ...
    if (isset($_POST['add'])) {
        ...
        $crypt=$_POST['crypt'];
        if ($res) {
            $update=$db->query("UPDATE cartes SET nom='$nom',num='$num',date_exp='$date_exp',crypt='$crypt' WHERE pseudo='$marque'");
        }
        else{
            $insert=$db->query("INSERT INTO cartes (pseudo,nom,num,date_exp,crypt) VALUES ('$marque','$nom','$num','$date_exp','$crypt')");
        }
